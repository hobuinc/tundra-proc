...
count = 0
limit = 10
bucket = S3Path.from_s3_uri('s3://grid-dev-lidarscans/Fairbanks-A-TLS/rxp/')



dtm_output_dir = "atls-dtm-cropped"
laz_output_dir = "atls-laz-classified"
...
for p in bucket.iter_objects().filter(S3Path.ext == ".gz"):
    ...
    if not exists(p, laz_output_dir, '.copc.laz'):
